...
enum Period {
     ...
     * @throws EelFunctionException is the {@code key} is not valid
     */
    @Nonnull
    static Period lookup(@Nonnull String key) throws EelFunctionException {
        Period period = LOOK_UP.get(key);

        if (period == null) {
            throw new EelFunctionException("Invalid date period '%s'", key);
        }
